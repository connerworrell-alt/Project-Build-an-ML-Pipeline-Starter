...
def test_row_count(data):
    # Verify that the dataset has a reasonable number of rows
    assert 15000 < data.shape[0] < 1000000, "Row count out of expected range"

def test_price_range(data, min_price, max_price):
    # Verify that all prices are between the given min and max
    assert data['price'].between(min_price, max_price).all(), "Price out of expected range"
